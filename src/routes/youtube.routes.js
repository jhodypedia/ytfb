...
router.post("/upload", auth(["user","admin"]), Ctrl.upload, (req,res)=> res.json({ ok:true, filePath:req.file.path }));
router.get("/", auth(["user","admin"]), Ctrl.list);
router.get("/:id", auth(["user","admin"]), Ctrl.detail);
router.post("/create", auth(["user","admin"]), detectClient, validate(createSchema.keys({ platform: createSchema.extract("platform").valid("youtube") })), Ctrl.create);
